...
export function ActorsEdit() {
  ...
  const { mutate } = useMutation({
    ...
    mutationFn: async (data: ActorFormData) => {
      if (!record) {
        throw new Error(
          translate("resources.actors.edit.record_not_found", {
            _: "Record not found",
          }),
        );
      }
    ...
    onSuccess: () => {
      redirect("/actors");
      notify("resources.actors.edit.success", {
        messageArgs: {
    ...
    onError: () => {
      notify("resources.actors.edit.error", {
        type: "error",
        messageArgs: {
...
const ActorEditTitle = () => {
  const record = useRecordContext<Actor>();
  const translate = useTranslate();
  if (!record) return null;
  return (
    <h2 className="text-lg font-semibold mb-4">
      {translate("resources.actors.edit.title", {
